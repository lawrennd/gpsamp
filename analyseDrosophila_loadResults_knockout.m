function [posteriors, linkMargPosteriors, linkPairPosteriors, priors, M, post_genes] = analyseDrosophila_loadResults_knockout()
...
load datasets/drosophila_knockouts;
load datasets/testset;

analyseDrosophila_constants_knockout;
...
if flag == 1     
    ...
    results_b = sortResults(load('results/multitf9uncons_2011-11-14_summary.mat'));
...
% number of TFs
M = drosMakeKnockoutValidationMatrix(drosmutant, results_b.genes);
% mask out the nans
mask = ones(size(results_b.genes,1),1); 
for i=1:size(results_b.genes,1)
    if isnan(sum( M(i,:) ))
...
prior32 = zeros(1, length(ind32));
for k=1:length(ind32),
  indOne = find(combConstr(ind32(k), mycols)==1);
  %prior(k)=0;
  for j=1:numGenes
      if all(M(j, indOne),2)  & sum(M(j, :),2)==1
...
withprior = results_b.marlls;
withprior(:, ind32) = withprior(:, ind32) + repmat(log(prior32), [size(withprior, 1), 1]);
...
posteriors{end+1} = baseline_a.marlls(:, indbase) - repmat(baseline_a.marlls(:, 1), ...
					   [1, length(indbase)]);

posteriors{end+1} = regression_a.marlls(:, ind32);
...
for l=1:30000
    ...
    Mprior = combConstr(ind32(sample_discrete(prior32)),mycols);
    if sum(Mrand(1,:),2) > 0 
...
%
  ...
  indSingle = ind32(find(combConstr(ind32,mycols(k))==1));
  ...
  linkMargPosteriors{3}(:, k) = zeros(size(linkMargPosteriors{1}(:, k)));
  linkMargPosteriors{4}(:, k) = zeros(size(linkMargPosteriors{1}(:, k)));
%  linkMargPosteriors{3}(:,k) = logsumexp(results_b.marlls(:, comb16(:, mycols(k))==1), 2) - ...
%                          logsumexp(results_b.marlls(:, comb16(:, mycols(k))==0), 2);
               
  % Posterior of the TF-link being active using the 16 models plus prior  
%  linkMargPosteriors{4}(:,k) = logsumexp(withprior(:, comb16(:, mycols(k))==1), 2) - ...
%                        logsumexp(withprior(:, comb16(:, mycols(k))==0), 2);
 
  % find the *single* index inside comb where only "k" is active 
  indIndiv = find(combConstr(:,ind32(k))==1 & sum(combConstr,2)==1 );     
  ...
  linkMargPosteriors{7}(:,k) = baseline_a.marlls(:, ind32(k+1)) - baseline_a.marlls(:, 1);
  ...
  linkMargPosteriors{8}(:,k) = max(abs([singles(:,ind32(k)), doubles(:, any(inf_pairs == ind32(k), 2))]), [], 2);
%  
end
...
for k=1:numTFs
  ...
  indPair = ind32(find(combConstr(ind32,mycols(k))==1 & combConstr(ind32,mycols(g))==1));
  ...
  indPairSingle = ind32(find(combConstr(ind32,mycols(k))==1 & combConstr(ind32,mycols(g))==1 & sum(combConstr(ind32, :),2)==2 ));
 
  linkPairPosteriors{3}(:, cnt) = zeros(size(linkPairPosteriors{1}(:, cnt)));
  linkPairPosteriors{4}(:, cnt) = zeros(size(linkPairPosteriors{1}(:, cnt)));
...
%			              logsumexp(withprior(:, setdiff(ind16, indPairSingle)), 2);
  ...
  %(0 0; k 0; 0 g; k g )
  ind4 = ind32(find(  sum(combConstr(ind32, setdiff(mycols, [mycols(k) mycols(g)]) ), 2)==0  ));
  ...
  indPSinglebase = find(baselinecomb(:,mycols(k))==1 & baselinecomb(:,mycols(g))==1 & sum(baselinecomb,2)==2 ); 
  ...
  indk = ind32(find(combConstr(ind32,mycols(k))==1  &  sum(combConstr(ind32,:),2)==1));
  indg = ind32(find(combConstr(ind32,mycols(g))==1  &  sum(combConstr(ind32,:),2)==1));
...
for k=1:size(doubles, 2),
  if any(inf_pairs(k,1) == mycols) && any(inf_pairs(k,2) == mycols),
    linkPairPosteriors{8}(:, all(bsxfun(@eq, inf_pairs(k,:), pairs_orig), 2)) = abs(doubles(:,k));
  end
end

% compute also Antti's doubles 
for i=1:numGenes
   for k=1:numTFs * (numTFs - 1) / 2,
       linkPairPosteriors{8}(i,k) = max(linkPairPosteriors{8}(i,k), min(abs(singles(i,ind32(pairs(k,1)))),  abs(singles(i,ind32(pairs(k,2))))));
   end
end
