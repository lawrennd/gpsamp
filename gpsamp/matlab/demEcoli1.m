...
%  time points)
clear;
load data/ecoliNormalisedData.mat
TimesG = times';
Genes(:,:,1) = y{1}'; 
namefile = 'RogersetalEcoli';
TimesG = times';
%%%%%%%%%%%%%%%%%%%%%% END LOAD DATA %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
if strcmp(modelOp.TFjointAct,'michMenten') == 1
    % set the TF to repress all genes
...
Gvariances = 'no';% 'yes' or 'no'. Do you know the gene variance values
...
trainOps.Burnin = 1000;  % burn in time
...
% (around 20 times more the number the discrete time points
% we have gene expressions) 
discr=20;
if (discr*size(TimesG,2)) > 200 
    discr = floor(200/size(TimesG,2)); 
end 
step = ((max(TimesG) - min(TimesG))/(size(TimesG(:),1)))/discr;
...
% Plot the results
plotCreate(model, samples, Genes, TimesG, TimesF, namefile, 0);
