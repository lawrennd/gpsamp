...
%				0*logsumexp(cvals_a(:,1),2), 'descend');
[foo, multitfrank1.twi] = sort(logsumexp(results_b.marlls(:,[4, 2]),2) - ...
		     logsumexp(results_b.marlls(:,[3, 1]),2), 'descend');
[foo, multitfrank1.mef2] = sort(logsumexp(results_b.marlls(:,[4, 3]),2) - ...
...
scores = logsumexp(results_b.marlls(:,posterior_indices),2) - ...
	 logsumexp(results_b.marlls(:, setdiff(1:4, posterior_indices)),2) - ...
	 mean(logsumexp(results_cb.marlls(:, posterior_indices, :), 2), 3) + ...
	 mean(logsumexp(results_cb.marlls(:, setdiff(1:4, posterior_indices), :), 2), 3);
